...
### Шаг 1: Управление Docker через CLI

Вместо графического интерфейса используйте прямые команды Docker:
```bash
...
| **Uptime Kuma (без Adminer)** | +300 MB | $0 | ⭐⭐⭐⭐⭐ | ⚠️ MEDIUM | ⚙️ Возможно |
...
1. Удалите Adminer если не используется (освободите ~50 MB)
